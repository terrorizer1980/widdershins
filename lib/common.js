...
function schemaToArray(schema,offset,options,data) {
    ...
    walkSchema(schema,{},wsState,function(schema,parent,state){
        ...
        const apiTarget = data.api.servers[0].url.split('/')[4] === "hackers" ? "hacker" : "customer";
        let entry = {};
        entry.schema = schema;
        entry.in = 'body';
        ...
        if (schema["x-widdershins-oldRef"]) {
            ...
            entry.safeType = '['+entry.$ref+'](/'+apiTarget+'-reference#'+entry.$ref.toLowerCase()+')';
        ...
        if ((entry.type === 'array') && schema.items) {
            ...
            if (schema.items["x-widdershins-oldRef"]) {
                ...
                itemsType = '['+$ref+'](/'+apiTarget+'-reference#'+$ref.toLowerCase()+')';
                if (!entry.description) {
