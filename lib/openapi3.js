...
function fakeProdCons(data) {
    const apiTarget = data.url.split('/')[4];
    data.authParameter = {};
    data.authParameter = apiTarget === "hackers" ? (
        {username:"username",token:"+RkfaGmptinbuqtURvzscSf/Sm4NXlw/x9pPtaLLzLE="}) : ({username:"api_example_company",token:"Ke+2jinhe5jM87P95aAVOz7L3ZWrtSiERtyOkkh5tEQ="});
    data.produces = [];
    data.consumes = [];
    data.bodyParameter = {};
...
function createSplitContent(templates, header, data, options, callback) {
    ...
    switch(sectionName) {
        case "customers":
            ...
            reference = createContent(`customer-reference`, templates, header, data, options, callback);
            break;
        case "hackers":
            resources = createContent(`hacker-resources`, templates, header, data, options, callback, options.omitHeader);
            reference = createContent(`hacker-reference`, templates, header, data, options, callback);
            break;
        default:
    }
...
function createContent(template, templates, header, data, options, callback) {
    let content = '';
